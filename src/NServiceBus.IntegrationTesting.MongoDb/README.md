# NServiceBus.IntegrationTesting.MongoDb

Adds a MongoDB container to [NServiceBus.IntegrationTesting](https://github.com/mauroservienti/NServiceBus.IntegrationTesting) test environments.
...
Add this package to your test project alongside `NServiceBus.IntegrationTesting`, then call `.UseMongoDB()` on `TestEnvironmentBuilder`:
...
The hostname `mongodb` is the container's name on the shared Docker network — endpoints reach it by that name without any extra configuration.
