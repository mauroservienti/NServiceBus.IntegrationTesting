namespace NServiceBus.AcceptanceTesting
{
    ...
    public class ScenarioWithContext<TContext> : IScenarioWithEndpointBehavior<TContext> where TContext : ScenarioContext, new()
    {
        ...
        void PrintLog(TContext scenarioContext)
        {
            ...
            {
                Console.WriteLine($"{logEntry.Timestamp:T} {logEntry.Level} {logEntry.Endpoint ?? "<unknown>"}: {logEntry.Message}");
            }
        }
        ...
        Func<ScenarioContext, Task<bool>> done = context => Task.FromResult(true);
    }
}